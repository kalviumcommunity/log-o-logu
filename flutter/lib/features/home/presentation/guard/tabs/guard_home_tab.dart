...
class GuardHomeTab extends StatelessWidget {
    ...
    return StreamBuilder<List<InviteModel>>(
      ...
      builder: (context, snapshot) {
        ...
        final readyNow = invites
            .where(
                (i) => now.isAfter(i.validFrom) && now.isBefore(i.validUntil))
            .toList()
        ...
        final expired = invites.where((i) => now.isAfter(i.validUntil)).length;
                        ...
                        children: [
                          Expanded(
                            child: FilledButton.icon(
                              onPressed: () => _openInviteValidationDialog(
                            ...
                            child: OutlinedButton.icon(
                              onPressed: () {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                        'QR scanner will be enabled in the next iteration.'),
                                  ),
                                );
                              },
